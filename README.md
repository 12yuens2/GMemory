...
`slurm/<task>_calibrate.sh` sits between the smoke test and a 24-hour job: one dataset, every arm, one seed, twenty tasks at the full trial budget, in a 2-hour allocation. `slurm/generate_calibration.py` generates them, from the same cluster configuration and job template as the sweep:
...
Either generator takes `--task`, and `SLURM_ACCOUNT=<account>` puts an `#SBATCH --account` line in whatever it writes; without it the scripts submit under your default account.
