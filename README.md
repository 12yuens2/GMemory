...
`slurm/<task>_calibrate.sh` sits between the smoke test and a 24-hour job: one dataset, every arm, one seed, twenty tasks at the full trial budget, in a 2-hour allocation. Generate them alongside the sweep scripts:

```bash
uv run slurm/generate_slurm.py --calibrate            # every dataset
uv run slurm/generate_slurm.py --calibrate --task fever pddl
sbatch slurm/fever_calibrate.sh
```

`SLURM_ACCOUNT=<account> uv run slurm/generate_slurm.py ...` puts an `#SBATCH --account` line in whatever it generates, sweep scripts included; without it they submit under your default account.

They default to `$HOME/GMemory/.db/sweep/calibration`, so calibrating several datasets fills one table. Each prints that table, the tokens per task and any failed tasks. That is what sizes the real jobs — every experiment in a job runs concurrently against one throughput-bound server, so the wall clock is total tokens divided by what the server sustains, and `tokens_per_task x episodes / throughput` is the estimate. Twenty tasks also takes `g-memory` past its twentieth, where `merge_insights` runs.
